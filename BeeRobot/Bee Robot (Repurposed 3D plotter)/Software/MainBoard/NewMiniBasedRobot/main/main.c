...
#define xdash(x,z)             x*cos(RotAng/100) + z*sin(RotAng/100)    // rotation x 
#define zdash(x,z)             z*cos(RotAng/100) - x*sin(RotAng/100)    // rotation z
...
void Rotateatapoint(float w_old,float speed, float x, float z,float phi)
{
    motor_timers_t mtt;
    w_cur = w_old + (speed / 1000000);
    //float exc_length = 23.0;
    p_old.x = params.exc_length*(cosf(w_old)); 
    p_old.z = params.exc_length*(sinf(w_old));

    p_cur.x = params.exc_length*(cosf(w_cur)); 
    p_cur.z = params.exc_length*(sinf(w_cur));

    mtt.motorx = -100000*(p_old.x - p_cur.x) + x;
    mtt.motorz = -100000*(p_old.z - p_cur.z) + z;
    mtt.motorphi = speed;
    if(mtt.motorx != 0)
        timer_start(TIMER_GROUP_1, TIMER_0);
    if(mtt.motorz != 0)
    ...
    if(mtt.motorphi != 0)
        timer_start(TIMER_GROUP_0, TIMER_1); 
    motorXZPHIDrive( mtt.motorx, mtt.motorz, mtt.motorphi);
}
    ...
    {   
        ...
        {   
            ...
            if(!(sysState&SYS_STATE_DOWN_ROBOT) && (DanceInitFlag & DSTATE_ANGLE_CORR)&& (keypad & KEY_D_ONOFF) )
            {
                ...
                //////////////////////////////////////*********************************
                    DanceInitFlag &= ~DSTATE_ANGLE_CORR;     //////////////////////////////Temporary please comment out///////////
                    
                    
                ////////////////////////////////////////*******************************
                if(DanceInitFlag & DSTATE_DIRECTION_CORR )
                {
                    Rotphi = params.angle + params.divergence/2.0;
                    if (Rotphi < 0) 
                        Rotphi += PI2;
                    Rot_dir_phi = -1.0;
                    diff = Rotphi - w_old;
                    ...
                    ESP_LOGI(TAG, "DIRECTION CORR %f",Rotphi );
                ...
                else if(DanceInitFlag & DSTATE_DIR_RETURN_CORR)
                {
                    ...
                    ESP_LOGI(TAG, "RETURN RUN CORR %f",Rotphi );
                ...
                {
                    if (Rot_dir_phi < 0)
                    Rot_dir_phi = -MIN_SET_ORIENTATION_SPEED;
                    else
                    Rot_dir_phi = MIN_SET_ORIENTATION_SPEED;
                }
                else
                {
                    if (Rot_dir_phi < 0)
                    Rot_dir_phi = -MAX_SET_ORIENTATION_SPEED;
                    else
                    Rot_dir_phi = MAX_SET_ORIENTATION_SPEED;
                }
               
                Rotateatapoint(w_old,Rot_dir_phi*(PI2),0,0,0);
            ...
            {   
                ...
                mtt.motorx= (int) (cosf(params.angle)*dance_speed_x[global_tick_count]- sinf(params.angle)*dance_speed_z[global_tick_count]) + motorX_Key_Offset*motorSpeedFactor;
                mtt.motorz =(int) (cosf(params.angle)*dance_speed_z[global_tick_count]+ sinf(params.angle)*dance_speed_x[global_tick_count])  + motorZ_Key_Offset*motorSpeedFactor;
                mtt.motorphi =  dance_speed_a[global_tick_count] + motorPHI_Key_Offset;   
                global_tick_count++;
                ...
                if (sysState & SYS_STATE_KEYBOARD_CTRL_ENABLED)
                {
                    if ( !(keypad & KEY_D_ONOFF) )
                    {
                            ...
                            rampFactor = 0.005;
                            motorXZPHIDrive(0,0,0);           // Motor Stop
            ...
            {    
                ...
                w_old = EncAng;
                Rotateatapoint(w_old,motorPHI_Key_Offset*(PI2),0,0,0);
                ...
                //ESP_LOGI(TAG, "val set  %d %d %d %f %f",mtt.motorx, mtt.motorz, mtt.motorphi, EncAng,w_cur);
            }
            ...
            {  
                ...
                else if ( (keypad & KEY_MODE) )
                {
                    ...
                    else if(keypad!=128)
                    {
                        ...
                        {
                            if(motorZ_Key_Offset == 0)
                                Rotphi = PI;
                            else if (motorZ_Key_Offset < 0)
                                Rotphi = PI*5/4;   
                            else
                                Rotphi = PI*3/4;    
                        }
                        ...
                        if(!(keypad & KEY_D_ONOFF))
                        {
                            if(!(limitSwitches[1][0]||limitSwitches[1][1]||limitSwitches[1][2]||limitSwitches[1][3]) && CurrMode== RotateMotion)
                            {
                                ...
                                else if (fabs(diff) < 0.15)
                                {
                                    ...
                                    Rotateatapoint(w_old,Rot_dir_phi*(PI2),motorX_Key_Offset*motorSpeedFactor,motorZ_Key_Offset*motorSpeedFactor,0);
                                }
                                else if (fabs(diff) > 0.15)
                                {
                                    if (Rot_dir_phi < 0)
